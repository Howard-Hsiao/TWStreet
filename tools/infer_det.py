...
def main():
    ...
    dt_boxes_json = [] ####
    model.eval()
    with open(save_res_path, 'wb') as fout:  # "wb"
        for file in get_image_file_list(config['Global']['infer_img']):
            ...
            if isinstance(post_result, dict):
                ...
                for k in post_result.keys():
                    ...
                    for box in boxes:
                        ...
                        dt_boxes_json.append( {"filename": file, "points": box.tolist()} )
                        #logger.info(len(dt_boxes_json))
            ...
            #otstr = json.dumps(dt_boxes_json) + "\n"
            #fout.write(otstr.encode())
            #save_det_path = os.path.dirname(config['Global']['save_res_path']) + "/det_results/"
            # draw_det_res(boxes, config, src_img, file, save_det_path)
    
        pickle.dump(dt_boxes_json, fout) #####
